...
def re_and_sm(disp_est, left, right):
    all_losses = []
...
def re_and_sm_loss(disp_ests, left, right):
    weights = [0.5, 0.5, 0.7, 1.0]
    all_losses = []
    for disp_est, weight in zip(disp_ests, weights):
        all_losses.append(weight * re_and_sm(disp_est, left, right))
    return sum(all_losses)



def generate_gradient_probability_mask(gradients, disp_gt, samples):
    #import pdb; pdb.set_trace()

    prob_only_valid = gradients.reshape(disp_gt.shape[1]*disp_gt.shape[2], -1) # [384*1248,1]
    mask = (disp_gt < 192) & (disp_gt > 0)
    valid_mask = mask.squeeze(0).cpu().reshape(disp_gt.shape[1]*disp_gt.shape[2], -1)
    valid_mask = valid_mask.squeeze(1)
    bad_inds, = np.where(valid_mask == 0)
    prob_only_valid[bad_inds] = 0.
    s = np.sum(prob_only_valid)

    prob_only_valid = prob_only_valid / s # probability
    prob_only_valid = prob_only_valid.squeeze(1)

    hints_inds = np.random.choice(disp_gt.shape[1]*disp_gt.shape[2], samples, False, prob_only_valid)
    pred_hints = torch.zeros_like(valid_mask)
    pred_hints[hints_inds] = 1
    pred_hints = pred_hints & valid_mask
    pred_hints = pred_hints.reshape(1, disp_gt.shape[1], disp_gt.shape[2])
